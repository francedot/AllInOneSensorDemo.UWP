...
using AllInOneSensorDemo.UWP.Extensions;

namespace AllInOneSensorDemo.UWP.Controls
{
    public sealed class SwipeableSplitView : SplitView
    {
        ...
        Selector _primaryMenuHost;
        Selector _secondaryMenuHost;
        IList<SelectorItem> _primaryMenuItems = new List<SelectorItem>();
        IList<SelectorItem> _secondaryMenuItems = new List<SelectorItem>();
        ...
        void OnManipulationDelta(object sender, ManipulationDeltaRoutedEventArgs e)
        {
            ...
            if (sender == _paneRoot && this.IsPanSelectorEnabled)
            {
                ...
                foreach (var item in _primaryMenuItems)
                {
                ...
                else if (_toBeSelectedIndex >= _primaryMenuItems.Count)
                {
                    _toBeSelectedIndex = _primaryMenuItems.Count - 1;
                }

                // highlight the item that's going to be selected
                var itemContainer = _primaryMenuItems[_toBeSelectedIndex];
        ...
        async void OnManipulationCompleted(object sender, ManipulationCompletedRoutedEventArgs e)
        {
            ...
            if (this.IsPanSelectorEnabled)
            {
                if (sender == _paneRoot)
                {
                    ...
                    _primaryMenuHost.SelectedIndex = _toBeSelectedIndex;
                }
                else
                {
                    // recalculate the starting distance
                    _startingDistance = _distancePerItem * _primaryMenuHost.SelectedIndex;
                }
            }
        }
        ...
        void OnPaneRootLoaded(object sender, RoutedEventArgs e)
        {
            ...
            if (this.IsPanSelectorEnabled)
            {
                ...
                _primaryMenuHost = (Selector) grid.Children.ElementAt(0);
                _secondaryMenuHost = (Selector) grid.Children.ElementAt(2);
                    
                    //GetChild<Selector>("For the bottom panning to work, the Pane's Child needs to be of type Selector.");

                foreach (var item in _primaryMenuHost.Items)
                {
                    var container = (SelectorItem)_primaryMenuHost.ContainerFromItem(item);
                    _primaryMenuItems.Add(container);
                }

                foreach (var item in _secondaryMenuHost.Items)
                {
                    var container = (SelectorItem)_secondaryMenuHost.ContainerFromItem(item);
                    _secondaryMenuItems.Add(container);
                }

                _distancePerItem = TOTAL_PANNING_DISTANCE / (_primaryMenuItems.Count);
